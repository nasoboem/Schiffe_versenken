...
import ( "os" ; "os/exec" ; "net" ; "fmt" ; "time" ; "strconv" )

const serververzeichnis string = "C:\\Go\\src\\gfx2\\gfx2server\\"
...
func SetzeServerprotokoll (w bool) {
	if w {
		if gfxServerAnfrage ("SPAN") != "OK" { panic ("Fehler!!") }
	} else {
		if gfxServerAnfrage ("SPAU") != "OK" { panic ("Fehler!!") }
	}
}
...
func Fenster (breite, hoehe uint16) {
	...
	prozess, err := start(serververzeichnis+"gfx2server", b, h, p, zielIP)
	if err != nil {
		fmt.Println (err)
...
func FensterOffen () bool {
	if anfragekanal == nil {return false}
...
func FensterAus () {
	...
	mauskanal = nil
	tastaturkanal = nil
	// ACHTUNG: "gfx2server" läuft noch und muss abgewürgt werden!!
	err:=serverprozess.Kill ()
	if err != nil { fmt.Println ("Fehler beim Serverkill:",err) }
...
func Grafikzeilen () uint16 {
	...
	if len(a) != 1 { panic ("Fehler!!") }
	ze,err := strconv.Atoi(a[0])
	if err != nil { panic ("Fehler beim 'Grafikzeilen'-Aufruf!") }
	return uint16(ze)
}
...
func Grafikspalten () uint16 {
	...
	if err != nil { panic ("Fehler beim 'Grafikspalten'-Aufruf!") }
	return uint16(sp)
}
...
func GibPunktfarbe (x,y uint16) (r,g,b uint8) {
	...
	a:=split(antw)
	if len(a) != 3 { panic ("Fehler!!") }
	red,err := strconv.Atoi(a[0])
	green,err2:= strconv.Atoi(a[1])
	blue, err3:= strconv.Atoi(a[2])
	if err != nil || err2 != nil || err3 != nil { panic ("Fehler beim 'GibPunktFarbe'-Aufruf!") }
...
func TastaturLesen1 () (taste uint16, gedrueckt uint8, tiefe uint16) {
	...
	if err != nil || err2 != nil || err3 != nil { panic ("Fehler beim 'TastaturLesen1'-Aufruf!") }
...
func Tastaturzeichen (taste, tiefe uint16) rune {
	...
	if err != nil { panic ("Fehler beim 'Tastaturzeichen'-Aufruf!") }
	return rune(ru)
}
...
func TastaturpufferLesen1 () (taste uint16, gedrueckt uint8, tiefe uint16) {
	...
	ge,err2:= strconv.Atoi(a[1])
	ti, err3:= strconv.Atoi(a[2])
	if err != nil || err2 != nil || err3 != nil { panic ("Fehler beim 'TastaturpufferLesen1'-Aufruf!") }
	taste,gedrueckt,tiefe = uint16(ta),uint8(ge),uint16(ti)
	return
}
...
func MausLesen1 () (taste uint8, status int8, mausX, mausY uint16) {
	...
	if len(a) != 4 { panic ("Fehler!!") }
	ta,err := strconv.Atoi(a[0])
	st,err2:= strconv.Atoi(a[1])
	mx, err3:= strconv.Atoi(a[2])
	my, err4:= strconv.Atoi(a[3])
	if err != nil || err2 != nil || err3 != nil || err4 != nil { panic ("Fehler beim 'MausLesen1'-Aufruf!") }
...
func MauspufferLesen1 () (taste uint8,status int8, mausX, mausY uint16) {
	...
	mx, err3:= strconv.Atoi(a[2])
	my, err4:= strconv.Atoi(a[3])
	if err != nil || err2 != nil || err3 != nil || err4 != nil { panic ("Fehler beim 'MauspufferLesen1'-Aufruf!") }
	taste,status,mausX,mausY = uint8(ta),int8(st),uint16(mx),uint16(my)
	return
}
...
func GibNotenTempo () uint8 {
	...
	if err != nil { panic ("Fehler beim 'GibNotenTempo'-Aufruf!") }
	return uint8(tempo)
}

func SetzeNotenTempo (t uint8) {
	a:=gfxServerAnfrage("SNTE:"+fmt.Sprint(t))
	if a[0:1]=="E" {panic ("Fehler bei 'SetzeNotenTempo' !") }
}
...
func GibKlangparameter () (r uint32, b uint8,k uint8,s uint8,p float64) {
	...
	if err != nil || err2 != nil || err3 != nil || err4 != nil || err5 != nil { panic ("Fehler beim 'GibKlangparameter'-Aufruf!") }
...
func SetzeKlangparameter(rate uint32, aufloesung,kanaele,signal uint8, p float64) {
	...
	if a[0:1]=="E" {panic ("Fehler bei 'SetzeKlangparameter' !") }
}
